...
class Uom extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}uom(name,code,description,status,created_at)values('$this->name','$this->code','$this->description','$this->status','$this->created_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}uom set name='$this->name',code='$this->code',description='$this->description',status='$this->status',created_at='$this->created_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,code,description,status,created_at from {$tx}uom");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,code,description,status,created_at from {$tx}uom $criteria limit $top,$perpage");
		$data=[];
		while($uom=$result->fetch_object()){
			$data[]=$uom;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,code,description,status,created_at from {$tx}uom where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($uom=$result->fetch_object()){
			...
			$html.="<tr><td>$uom->id</td><td>$uom->name</td><td>$uom->code</td><td>$uom->description</td><td>$uom->status</td><td>$uom->created_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Code</th><td>$uom->code</td></tr>";
		$html.="<tr><th>Description</th><td>$uom->description</td></tr>";
		$html.="<tr><th>Status</th><td>$uom->status</td></tr>";
