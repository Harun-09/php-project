...
public static function find_report($startDate = '', $endDate = '', $productId = '') {
    ...
    $sql = "SELECT 
                ...
                SUM(s.qty) AS balance,
                SUM(CASE WHEN s.qty > 0 THEN s.qty ELSE 0 END) AS total_in,
                SUM(CASE WHEN s.qty < 0 THEN -s.qty ELSE 0 END) AS total_out
            FROM {$tx}stocks s
            JOIN {$tx}products p ON s.product_id = p.id
            WHERE 1=1";

    if (!empty($productId)) {
        $sql .= " AND s.product_id = " . intval($productId);
    }

    if (!empty($startDate)) {
        $sql .= " AND s.created_at >= '" . $db->real_escape_string($startDate) . " 00:00:00'";
    }

    if (!empty($endDate)) {
        $sql .= " AND s.created_at <= '" . $db->real_escape_string($endDate) . " 23:59:59'";
    }
    ...
    if ($result) {
        while ($row = $result->fetch_object()) {
            $row->total_in = (float)$row->total_in;
            $row->total_out = (float)$row->total_out;
